...
if isequal(log_file, '0') || isequal(log_file, 'Select File')
    add_check = 0;
    new_check = 1;
...
if class(x) ~= 'cell'
    if strcmp(x, '0') || strcmp(x, 'Select Files')  
...
out = filegetter(handles, get(handles.addtable, 'String'), filename, log_file, handles.addtable); %makes sure a file is selected
log_file = string(out);
set(handles.addtable,'String', log_file);
